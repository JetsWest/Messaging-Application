...
public class ChainingHashTableCopy<K, V> extends DeletelessDictionary<K, V> {
    ...
    public static int CHTsteps = 0;

    public ChainingHashTableCopy(Supplier<Dictionary<K, V>> newChain) {
    ...
    public V insert(K key, V value) {
        ...
        CHTsteps += ((MoveToFrontList) this.array[index]).getSteps();
        reset(index);
        if (result == null) {
            this.size++;
        }
    ...
    public V find(K key) {
        ...
        if (array[index] == null) {
            ...
            this.CHTsteps += ((MoveToFrontList)this.array[index]).getSteps();
            reset(index);
    ...
    public void reset(int i) {
        ((MoveToFrontList)this.array[i]).MTFLsteps = 0;
    }
        ...
        for (int i = 0; i < array.length; i++) {
            if (array[i] != null) {
                for (Item<K, V> item : array[i]) {
                    int newhash = Math.abs(item.key.hashCode() % newArr.length);
